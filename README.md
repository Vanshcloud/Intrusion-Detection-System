...
7. **Novel-vector performance stays high, but that subset has a different attack mix.** On the 511,247 test rows whose feature vector never appears in training, recall is 0.99937 and FPR 0.645 %. Scans make up 45.7 % of attack rows in the full test set but only 2.4 % in the novel subset, so the two numbers are not directly comparable. A novel vector is not an unseen or zero-day attack: every novel test row belongs to an attack type, tool and host that also appear in training.
...
- when it was exported, its scores were bit-identical to the frozen model on all 629,992 test rows, and its SHAP values were bit-identical on the 118 locally held explorer rows; with the local artifacts, `scripts/m6_export_model.py --check` repeats this audit.
